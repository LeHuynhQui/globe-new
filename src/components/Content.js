...
import dataCapital from "./../data/dataCapital.json"
...
export default function Content() {
    ...
    useEffect(() => {
        document.querySelector("#rotation").addEventListener("mousedown", e => {
            document.querySelectorAll("circle").forEach(around => {
                    ...
                    .on("click", function (d) {
                        ...
                        mangTong.forEach(nation => {
                            ...
                            if (nation.name.toLowerCase().replace(/ /g, '').replaceAll(".", "").replaceAll("'", "") === dName) {
                                ...
                                codeNationDuocChon = nation.alpha2
                        ...
                        //LAY REGION
                        // console.log("duoc chon", d)
                        // console.log("codeNationDuocChon", codeNationDuocChon)

                        let regionName = null
                        for (let keys in regions) {
                            regions[keys].countries.map(coun => {
                                if (coun === codeNationDuocChon) {
                                    regionName = keys
                                }
                                return null
                            })
                            ...
                            region.countries.map(code => {
                                dataCapital.map(country => {
                                    if (country.alpha2Code === code) {
                                        ...
                                        capitalOnClick.push(objectCountry)
                                    }
                                return null
                        ...
                        // LAM CHO REGION SANG LEN
                        capitalOnClick.map(coun => {
                            let className = coun.name.toLowerCase().replaceAll("(","").replaceAll(")","").replaceAll(" ","").replace(/ /g,'')
                            if (className === "laopeople'sdemocraticrepublic") {
                                className = "laos"
                            }
                
                            if (className === "timor-leste,democraticrepublicof") {
                                className = "timor-leste"
                            }
                
                            if (className === "cocos(keeling)islands") {
                                className = "timor-leste"
                            }
                            if (className === "bruneidarussalam") {
                                className = "brunei"
                            }
                
                            if (className === "bosnia&herzegovina") {
                                className = "bosniaandherz"
                            }

                            if (className === "westernsahara") {
                                className = "wsahara"
                            }
                            
                            if (className === "centralafricanrepublic") {
                                className = "centralafricanrep"
                            }

                            if (className === "republicofcongo") {
                                className = "congo"
                            }
                
                            if (className === "democraticrepublicofcongo") {
                                className = "demrepcongo"
                            }
                            

                            if (className === "equatorialguinea") {
                                className = "eqguinea"
                            }
                            
                            if (className === "virginislands(british)") {
                                className = "anguilla"
                            }
    
                            if (className === "virginislands(us)") {
                                className = "anguilla"
                            }

                            if (className === "dominicanrepublic") {
                                className = "dominicanrep"
                            }
                            
                            if (className === "syrianarabrepublic") {
                                className = "syria"
                            }
                            
                            if (className === "côted'ivoire") {
                                className = "côtedivoire"
                            }

                            if (className === "unitedstates") {
                                className = "unitedstatesofamerica"
                            }

                            if (className === "swaziland") {
                                className = "eswatini"
                            }

                            if (className === "falklandislands") {
                                className = "falklandis"
                            }

                            if (className === "frenchguiana") {
                                className = "trinidadandtobago"
                            }
                            ...
                            if (className === "southsudan") {
                                className = "ssudan"
                            }
                            
                            if (className === "solomonislands") {
                                className = "solomonis"
                            }
                            
                            if (className === "korea,democraticpeople'srepublicof") {
                                className = "northkorea"
                            }
                
                            if (className === "korea,republicof") {
                                className = "southkorea"
                            }

                            if (className === "heardislandandmcdonaldislands") {
                                className = "frsantarcticlands"
                            }
                            
                            if (className === "palestinianterritory,occupied") {
                                className = "palestine"
                            }


                            if(document.querySelector(`.${className}`)) {
                                document.querySelector(`.${className}`).classList.add("region")
                            }
                            return null
